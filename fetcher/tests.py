...
class test_add(TestCase):
    ...
    def test_invalid_title_post(self):
        ...
        response = client.post('http://127.0.0.1:8001/add_item',
                               {'title': '', 'description': 'helloali', 'image': 'xx.jpg'})
    ...
    def test_invalid_description_post(self):
        client = RequestsClient()
        response = client.post('http://127.0.0.1:8001/add_item',
                               {'title': 'test', 'description': '', 'image': 'xx.jpg'})
    ...
    def test_valid_post(self):
        client = RequestsClient()
        response = client.post('http://127.0.0.1:8001/add_item',
                               {'title': 'test', 'description': 'test', 'image': 'test'})
        assert response.status_code == 201
...
class test_update(TestCase):
    def test_invalid_update(self):
        client = RequestsClient()
        response = client.put('http://127.0.0.1:8001/update/1',
                              {'title': 'test1', 'description': 'test1', 'image': 'xx.jpg'})
        assert response.status_code == 202
